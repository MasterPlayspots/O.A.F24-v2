...
export const cases = new Hono<{ Bindings: Bindings; Variables: Variables }>();

// ============================================
// Cases: Workflow engine
// ============================================

cases.post("/cases", requireAuth, async (c) => {
...
// List user's cases
cases.get("/cases", requireAuth, async (c) => {
  const user = c.get("user");
  const bafaDb = c.env.BAFA_DB;
  const foerderDb = c.env.FOERDER_DB;
...
cases.get("/cases/:id", requireAuth, async (c) => {
  const user = c.get("user");
...
// Complete a step
cases.patch("/cases/:caseId/steps/:stepId", requireAuth, async (c) => {
  const user = c.get("user");
...
cases.post("/cases/:caseId/dokumente", requireAuth, async (c) => {
  const user = c.get("user");
...
cases.get("/cases/:caseId/dokumente", requireAuth, async (c) => {
  const user = c.get("user");
  const caseId = c.req.param("caseId");
  const bafaDb = c.env.BAFA_DB;
