...
// ============================================================
// BAFA-Cert Upload (GAP-002) — Berater reicht BAFA-Zertifikat als PDF ≤ 5 MB
// ein. Gespeichert unter `{userId}/{ts}-{sanitized}` in R2 BAFA_CERTS,
// setzt users.bafa_cert_status='pending'. Admin prüft anschließend in der
// Cert-Queue (GAP-001).
// ============================================================
...
function sanitizeCertFilename(name: string): string {
...
berater.post("/bafa-cert", requireAuth, requireRole("berater"), async (c) => {
  ...
  if (
    typeof bafaBeraterNrRaw !== "string" ||
    !BAFA_BERATER_NR_PATTERN.test(bafaBeraterNrRaw.trim())
  ) {
    return c.json(
      {
        success: false,
  ...
  const safeName = sanitizeCertFilename(file.name || "cert.pdf");
