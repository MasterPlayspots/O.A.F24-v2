...
export async function searchKatalog(
  ...
  const dataResult = await foerderDb
    ...
    .all<FoerderprogrammRow & { total_count: number }>();
  const total =
    dataResult.results && dataResult.results.length > 0
      ? (dataResult.results[0]?.total_count ?? 0)
      : 0;
...
export async function getFilterOptions(foerderDb: D1Database): Promise<FilterOptions> {
  ...
  return {
    ...
    foerderbereich: (bereichResult?.results ?? []).map(
      (r) => (r as Record<string, unknown>).foerderbereich
    ),
    foerdergebiet: (gebietResult?.results ?? []).map(
      (r) => (r as Record<string, unknown>).foerdergebiet
    ),
    foerderberechtigte: (berechtigteResult?.results ?? []).map(
      (r) => (r as Record<string, unknown>).foerderberechtigte
    ),
  };
}
...
export async function getProgramById(
  id: number,
  foerderDb: D1Database
): Promise<FoerderprogrammRow | null> {
...
export async function getUserProfile(
  userId: string,
  bafaDb: D1Database
): Promise<FoerdermittelProfileRow | null> {
...
export async function getMatchesForProfile(
  ...
  Array<
    FoerdermittelMatchRow & {
      match_reasons: string[];
      disqualifiers: string[];
      programm: FoerderprogrammRow | null;
    }
  >
> {
...
Antworte NUR mit dem JSON.`;
  ...
  await bafaDb
    ...
    .bind(templateId, programId, phasesJson, generatedBy)
    .run();
...
export async function createCase(
  ...
  await bafaDb
    ...
    .bind(caseId, matchId ?? null, profileId, programId)
    .run();
...
export async function updateCaseStep(
  ...
  await bafaDb
    ...
    .bind(status, resultDataStr, status, stepId, caseId)
    .run();

  // Check if all required steps in current phase are completed → advance phase
  const cs = await bafaDb
    .prepare("SELECT id, phase FROM foerdermittel_cases WHERE id = ?")
    .bind(caseId)
...
VORAUSSETZUNGEN: ${(program.rechtliche_voraussetzungen || "").slice(0, 2000)}`;
  ...
  if (conversation) {
    ...
    await bafaDb
      ...
      .bind(convId, caseId ?? null, profile.id, context, JSON.stringify(messages))
      .run();
