...
export async function verifyEmail(code: string, token: string): Promise<{ ok: boolean }> {
...
export async function resendVerification(token: string): Promise<{ ok: boolean }> {
  return apiCall<{ ok: boolean }>(API.CHECK, '/api/auth/resend-verification', {
    method: 'POST',
  }, token)
}
...
export async function logout(token: string): Promise<{ ok: boolean }> {
  // Client-seitig Cookie löschen:
  if (typeof document !== 'undefined') {
    document.cookie = 'fund24-auth=; path=/; max-age=0'
  }
  return apiCall<{ ok: boolean }>(API.CHECK, '/api/auth/logout', {
    method: 'POST',
  }, token)
}
