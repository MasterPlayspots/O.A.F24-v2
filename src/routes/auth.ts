...
function setAuthCookies(
  ...
  c.header("Set-Cookie", `access_token=${accessToken}; ${cookieOpts}; Max-Age=1800`, {
    append: true,
  });
  c.header("Set-Cookie", `refresh_token=${refreshToken}; ${cookieOpts}; Max-Age=604800`, {
    append: true,
  });
}
...
auth.post("/register", registerRateLimit, async (c) => {
  ...
  await db
    ...
    .bind(verificationCode, codeExpires, id)
    .run();
...
auth.post("/login", loginRateLimit, async (c) => {
  ...
  if (!user.email_verified) {
    ...
    await db
      ...
      .bind(verificationCode, codeExpires, user.id)
      .run();
    if (c.env.RESEND_API_KEY) {
      await sendVerificationCodeEmail(
        c.env.RESEND_API_KEY,
  ...
  await db.batch([
    ...
    db
      ...
      .bind(crypto.randomUUID(), user.id, refreshHash, expiresAt),
  ]);
...
auth.post("/refresh", async (c) => {
  ...
  await db.batch([
    ...
    db
      ...
      .bind(crypto.randomUUID(), row.id, newHash, expiresAt),
  ]);
...
auth.post("/verify-email", async (c) => {
  ...
  const user = await c.env.DB.prepare("SELECT id FROM users WHERE verification_token = ?")
    .bind(token)
    .first<{ id: string }>();
...
auth.post("/resend-code", registerRateLimit, async (c) => {
  ...
  await db
    ...
    .bind(newCode, codeExpires, user.id)
    .run();
  if (c.env.RESEND_API_KEY) {
    await sendVerificationCodeEmail(
      c.env.RESEND_API_KEY,
...
auth.get("/me", requireAuth, async (c) => {
  const u = (await c.env.DB.prepare("SELECT * FROM users WHERE id = ?")
    .bind(c.get("user").id)
    .first()) as UserRow | null;
  if (!u) return c.json({ success: false, error: "Nicht gefunden" }, 404);
  let profile = null;
  if ((u as any).profile_json) {
    try {
      profile = JSON.parse((u as any).profile_json);
    } catch {
      /* ignore */
    }
  }
...
// PUT /me/profile — persist onboarding profile (agency, branding, specialization)
auth.put("/me/profile", requireAuth, async (c) => {
  const body = await c.req.json();
  const profileJson = JSON.stringify(body);
  if (profileJson.length > 50000) {
    return c.json({ success: false, error: "Profildaten zu groß" }, 400);
  }
  await c.env.DB.prepare(
    "UPDATE users SET profile_json = ?, onboarding_completed = 1, updated_at = datetime('now') WHERE id = ?"
  )
    .bind(profileJson, c.get("user").id)
    .run();
  return c.json({ success: true });
});
