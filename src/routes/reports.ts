...
import { generateReportSection, generateReportSectionStream } from "../services/ai";
...
reports.post("/generate", requireAuth, generateRateLimit, async (c) => {
  ...
  const fallbackTexts: Record<string, string> = {
    ausgangslage:
      "[Dieser Abschnitt konnte nicht automatisch generiert werden. Bitte ergänzen Sie die Ausgangslage manuell.]",
    beratungsinhalte: "[Bitte ergänzen Sie die Beratungsinhalte manuell.]",
    massnahmen: "[Bitte ergänzen Sie den Maßnahmenplan manuell.]",
    ergebnisse: "[Bitte manuell ergänzen.]",
    nachhaltigkeit: "[Bitte manuell ergänzen.]",
  };
  ...
  for (const phase of phases) {
    try {
      ...
      if (!r.success) {
        log("warn", "reports_generate_ai_section_fallback", { phase });
        fallbackSections.push(phase);
        results[phase] = { success: true, text: fallbackTexts[phase] };
      } else {
        results[phase] = r;
      }
    } catch (err) {
      log("error", "reports_generate_ai_section_threw", {
        phase,
  ...
  const response: Record<string, any> = {
    ...
    message:
      fallbackSections.length > 0
        ? "Bericht teilweise generiert – einige Abschnitte müssen manuell ergänzt werden"
        : "Bericht erfolgreich generiert",
  };
  if (fallbackSections.length > 0) {
    response.warnings = fallbackSections.map(
      (s) => `Abschnitt "${s}" konnte nicht generiert werden und enthält einen Platzhalter.`
    );
    response.fallbackSections = fallbackSections;
  }
...
reports.post("/unlock", async (c) => {
  ...
  try {
    ...
    if (report.is_unlocked === 1)
      return c.json({ success: true, message: "Bereits freigeschaltet" });
    ...
    log("error", "reports_unlock_error", { error: e instanceof Error ? e.message : String(e) });
...
reports.post("/:id/finalize", requireAuth, async (c) => {
  ...
  try {
    ...
    log("error", "reports_finalize_error", { error: e instanceof Error ? e.message : String(e) });
    return c.json({ success: false, error: "Finalisierung fehlgeschlagen" }, 500);
  }
});

// POST /:id/outcome — Record BAFA approval/rejection for reverse learning
const outcomeSchema = z.object({
  outcome: z.enum(["approved", "rejected"]),
  feedback: z.string().optional(),
});

reports.post("/:id/outcome", requireAuth, async (c) => {
  const antragId = c.req.param("id");
  const user = c.get("user");

  // Verify ownership via zfbf-db
  const ownership = await c.env.DB.prepare("SELECT id FROM reports WHERE id = ? AND user_id = ?")
    .bind(antragId, user.id)
    .first<{ id: string }>();
  if (!ownership) return c.json({ success: false, error: "Bericht nicht gefunden" }, 404);
  ...
  // Look up branche from BAFA_DB antrag
  const antrag = await c.env.BAFA_DB.prepare("SELECT branche_id FROM antraege WHERE id = ?")
    .bind(antragId)
    .first<{ branche_id: string | null }>();

  const { outcome, feedback } = parsed.data;
  const learningId = crypto.randomUUID();

  try {
    await c.env.BAFA_CONTENT.prepare(
      `INSERT INTO bafa_learnings (id, report_id, outcome, feedback, branche, created_at)
       VALUES (?, ?, ?, ?, ?, datetime('now'))`
    )
      .bind(learningId, antragId, outcome, feedback || null, antrag?.branche_id || null)
      .run();

    log("info", "report_outcome_recorded", {
      reportId: antragId,
      outcome,
      branche: antrag?.branche_id,
    });
    return c.json({ success: true, learningId });
  } catch (e) {
    log("error", "report_outcome_error", { error: e instanceof Error ? e.message : String(e) });
    return c.json({ success: false, error: "Outcome konnte nicht gespeichert werden" }, 500);
  }
});

// POST /generate-stream — SSE streaming for a single section
const streamSchema = z.object({
  ...
  massnahmen: z
    .array(z.object({ name: z.string(), methode: z.string(), ergebnis: z.string() }))
    .optional(),
  phase: z.enum(["ausgangslage", "beratungsinhalte", "massnahmen", "ergebnisse", "nachhaltigkeit"]),
});

reports.post("/generate-stream", requireAuth, generateRateLimit, async (c) => {
  const parsed = streamSchema.safeParse(await c.req.json());
  if (!parsed.success) {
    return c.json({ success: false, error: "Ungültige Parameter" }, 400);
  }

  const stream = await generateReportSectionStream(c.env.AI, parsed.data);
