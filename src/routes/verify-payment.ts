...
verifyPayment.post("/verify-payment", async (c) => {
  ...
  if (provider === "stripe") {
    ...
    try {
      ...
      log("error", "verify_payment_stripe_lookup_error", {
        error: e instanceof Error ? e.message : String(e),
      });
      return c.json({ success: false, error: "Zahlungsüberprüfung fehlgeschlagen" }, 500);
    }
  }
  ...
  if (provider === "paypal") {
    ...
    try {
      ...
      log("error", "verify_payment_paypal_error", {
        error: e instanceof Error ? e.message : String(e),
      });
      return c.json({ success: false, error: "Zahlungsüberprüfung fehlgeschlagen" }, 500);
    }
