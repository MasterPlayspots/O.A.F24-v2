...
import { z } from 'zod'
import type { Bindings, Variables, FoerderprogrammRow, FoerdermittelProfileRow, FoerdermittelMatchRow, FoerdermittelCaseRow, FoerdermittelCaseStepRow, FoerdermittelFunnelTemplateRow, FoerdermittelDokumentRow, FoerdermittelConversationRow } from '../types'
...
foerdermittel.get('/katalog', async (c) => {
  ...
  const { page, limit, foerderart, foerderbereich, foerdergebiet, foerderberechtigte, search, sort } = parsed.data
...
foerdermittel.get('/profile', requireAuth, async (c) => {
  ...
  ).bind(user.id).first<FoerdermittelProfileRow>()
...
foerdermittel.get('/matches', requireAuth, async (c) => {
  ...
  ).bind(profile.id).all<FoerdermittelMatchRow>()
...
foerdermittel.post('/cases', requireAuth, async (c) => {
  ...
  ).bind(body.programm_id).first<FoerdermittelFunnelTemplateRow>()
...
Antworte NUR mit dem JSON.`
    ...
    try {
      ...
      await bafaDb.prepare(
        `INSERT INTO foerdermittel_funnel_templates (id, programm_id, phases, generated_by)
         VALUES (?, ?, ?, ?)`
      ).bind(templateId, body.programm_id, phasesJson, '@cf/meta/llama-3.1-8b-instruct').run()
      ...
      await bafaDb.prepare(
        `INSERT INTO foerdermittel_funnel_templates (id, programm_id, phases, generated_by)
         VALUES (?, ?, ?, 'fallback')`
      ).bind(templateId, body.programm_id, defaultPhases).run()
  ...
  ).bind(caseId, body.match_id ?? null, profile.id, body.programm_id).run()

  // Copy template steps into case_steps
  if (template) {
    const phases = JSON.parse(template.phases) as Array<{
      id: string; title: string; steps: Array<{ title: string; description: string; type: string; required: boolean }>
    }>
...
foerdermittel.get('/cases', requireAuth, async (c) => {
  ...
  ).bind(profile.id).all<FoerdermittelCaseRow>()
...
foerdermittel.patch('/cases/:caseId/steps/:stepId', requireAuth, async (c) => {
  ...
  const cs = await bafaDb.prepare(
    'SELECT id, phase FROM foerdermittel_cases WHERE id = ? AND profile_id = ?'
  ).bind(caseId, profile.id).first<{ id: string; phase: string }>()
  if (!cs) return c.json({ success: false, error: 'Fall nicht gefunden' }, 404)
  ...
  ).bind(newStatus, resultData, newStatus, stepId, caseId).run()
...
foerdermittel.post('/chat', requireAuth, async (c) => {
  ...
  if (conversation_id) {
    ...
    ).bind(conversation_id, profile.id).first<FoerdermittelConversationRow>()
...
UNTERNEHMEN: ${profile.company_name}, Branche: ${profile.branche || 'k.A.'}, Standort: ${profile.standort || 'k.A.'}, Rechtsform: ${profile.rechtsform || 'k.A.'}`
  ...
  if (case_id) {
    const cs = await bafaDb.prepare(
      'SELECT * FROM foerdermittel_cases WHERE id = ? AND profile_id = ?'
    ).bind(case_id, profile.id).first<FoerdermittelCaseRow>()
  ...
  if (conversation) {
    ...
    await bafaDb.prepare(
      `INSERT INTO foerdermittel_conversations (id, case_id, profile_id, context, messages)
       VALUES (?, ?, ?, ?, ?)`
    ).bind(convId, case_id ?? null, profile.id, context, JSON.stringify(messages)).run()
