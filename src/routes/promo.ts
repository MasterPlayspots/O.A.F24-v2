...
promo.post("/validate", async (c) => {
  ...
  return c.json({
    ...
    discount: {
      ...
      code: g.code,
      remainingUses: g.max_uses - g.total_uses,
    },
  });
});
...
promo.post("/redeem", requireAuth, async (c) => {
  ...
  const insertResult = await db
    .prepare(
      "INSERT OR IGNORE INTO promo_redemptions (id, user_id, promo_code_id, order_id, discount_amount) VALUES (?, ?, ?, ?, ?)"
    )
    .bind(redemptionId, user.id, g.id, orderId || null, g.discount_value)
    .run();
  ...
  const updateResult = await db
    .prepare(
      "UPDATE gutscheine SET total_uses = total_uses + 1 WHERE id = ? AND total_uses < max_uses"
    )
    .bind(g.id)
    .run();
  ...
  await writeAuditLog(db, {
    userId: user.id,
    eventType: AUDIT_EVENTS.PROMO_REDEEM,
    detail: `${code} (${g.discount_type}: ${g.discount_value})`,
    ip: c.req.header("CF-Connecting-IP"),
  });
  return c.json({
    success: true,
    redemptionId,
    discount: { type: g.discount_type, value: g.discount_value },
  });
});
...
promo.post("/create", requireAuth, requireRole("admin"), async (c) => {
  ...
  const existing = await c.env.DB.prepare("SELECT id FROM gutscheine WHERE code = ?")
  ...
  await c.env.DB.prepare(
    "INSERT INTO gutscheine (id, code, discount_type, discount_value, max_uses, valid_from, valid_until, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
  )
    .bind(
      id,
      code.toUpperCase(),
