...
payments.post("/stripe/create-session", requireAuth, async (c) => {
  ...
  const antrag = await c.env.BAFA_DB.prepare("SELECT unternehmen_name FROM antraege WHERE id = ?")
    .bind(reportId)
  ...
  if (amount === 0) {
    await db
      .prepare("UPDATE reports SET is_unlocked = 1, unlock_payment_id = 'promo-free' WHERE id = ?")
      .bind(reportId)
      .run();
  ...
  try {
    ...
    await db
      .prepare(
        "INSERT INTO payments (id, user_id, report_id, package_type, amount, provider, provider_payment_id, gutschein_code, status) VALUES (?, ?, ?, 'einzel', ?, 'stripe', ?, ?, 'pending')"
      )
      .bind(crypto.randomUUID(), user.id, reportId, amount, session.id, promoCode || null)
      .run();
...
payments.post("/stripe/webhook", async (c) => {
  ...
  try {
    if (event.type === "checkout.session.completed") {
      ...
      if (reportId && userId) {
        await db
          .prepare("UPDATE payments SET status = 'completed' WHERE provider_payment_id = ?")
          .bind(s.id)
          .run();
        await db
      ...
      if (pay?.report_id) {
        await db.batch([
          ...
          db
            .prepare("UPDATE payments SET status = 'refunded' WHERE provider_payment_id = ?")
            .bind(charge.payment_intent),
        ]);
        await bafaDb
          .prepare(
    ...
    log("error", "payments_webhook_d1_error", {
...
payments.post("/paypal/create-order", requireAuth, async (c) => {
  ...
  try {
    ...
    await db
      .prepare(
        "INSERT INTO payments (id, user_id, report_id, package_type, amount, provider, provider_payment_id, status) VALUES (?, ?, ?, 'einzel', ?, 'paypal', ?, 'pending')"
      )
      .bind(crypto.randomUUID(), user.id, reportId, REPORT_PRICE_CENTS, order.orderId)
      .run();
