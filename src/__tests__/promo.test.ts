...
beforeAll(async () => {
  ...
  await env.DB.batch([
    env.DB.prepare("INSERT OR IGNORE INTO gutscheine (id, code, discount_type, discount_value, max_uses, total_uses, is_active) VALUES ('promo-1', 'TEST20', 'percent', 20, 100, 0, 1)"),
    env.DB.prepare("INSERT OR IGNORE INTO gutscheine (id, code, discount_type, discount_value, max_uses, total_uses, is_active) VALUES ('promo-2', 'EXPIRED', 'percent', 10, 100, 100, 1)"),
    env.DB.prepare("INSERT OR IGNORE INTO gutscheine (id, code, discount_type, discount_value, max_uses, total_uses, is_active) VALUES ('promo-3', 'INACTIVE', 'percent', 15, 100, 0, 0)"),
    env.DB.prepare("INSERT OR IGNORE INTO gutscheine (id, code, discount_type, discount_value, max_uses, total_uses, is_active) VALUES ('promo-4', 'FIXED50', 'fixed', 5000, 50, 0, 1)"),
  ])
})
...
describe('POST /api/promo/redeem', () => {
  ...
  it('prevents double redemption by same user', async () => {
    ...
    await env.DB.prepare('INSERT INTO promo_redemptions (id, user_id, promo_code_id) VALUES (?, ?, ?)')
      .bind(crypto.randomUUID(), userId, 'promo-1').run()
...
describe('Admin Promo Routes', () => {
  it('allows admin to list promo codes', async () => {
    const adminId = await createTestUser(env.DB, { email: 'promo-admin@example.com', role: 'admin' })
  ...
  it('allows admin to create promo code', async () => {
    const adminId = await createTestUser(env.DB, { email: 'promo-create@example.com', role: 'admin' })
  ...
  it('rejects non-admin from listing codes', async () => {
    const userId = await createTestUser(env.DB, { email: 'promo-nonadmin@example.com', role: 'user' })
